 ...
 * @author Eric
 */
@WebServlet(name = "ModificarPlanAccept", urlPatterns = {"/ModificarPlanAccept"})
public class ModificarPlanAccept extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<head>");
            out.println("<title>Modificar plan aceptado</title>"); 
            out.println("<link rel='stylesheet' type='text/css' href='resource/index.css'/>");
            out.println("</head>");
            out.println("<body>");
            out.println("<div class='center'>");
            out.println("<h1>Cambio de plan realizado con éxito</h1>");
            
            String dni = request.getParameter("dni");
            Usuario us = conEJB.findUsuarioByDni(dni);
            TramiteAltas tra = conEJB.findAltaByDni(us);
            tra.setTipoPlan(request.getParameter("tiplan"));
            tra.setPrecio(Integer.parseInt(request.getParameter("precio")));
            conEJB.modificarPlan(tra);
