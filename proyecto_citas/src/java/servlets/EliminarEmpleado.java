...
public class EliminarEmpleado extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<head>");
            out.println("<title>Eliminar empleado</title>");
            out.println("<link rel='stylesheet' type='text/css' href='resource/index.css'/>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<div class='center'>");
            if(conEJB.existeEmpleado(new Empleado(request.getParameter("dni")))){
                conEJB.eliminarEmpleado(request.getParameter("dni"));
                out.println("<h1>EMPLEADO ELIMINADO DE LA BASE DE DATOS</h1>");
            }else{
                out.println("<h1>ESTE EMPLEADO NO EXISTE EN LA BASE DE DATOS O DNI INCORRECTO</h1>");
            }
